...
```bash
dephell venv create --env {flake}
dephell deps install --env {flake}
dephell venv run --env {flake}
```
...
def make_contributing(config: Dict[str, Dict[str, Any]]) -> Optional[str]:
    contributing_parts = [
        CONTRIBUTING, BASICS,
        TESTING.format(pytest=config['pytest']['command'].split()[2]),
        STYLE.format(flake=config['flake8']['command'],
                     isort=config['isort']['command'].split()[0]),
        IDE, LINKS]

    return ''.join(contributing_parts)
